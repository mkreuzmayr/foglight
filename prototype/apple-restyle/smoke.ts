...
const dom = new JSDOM("<!doctype html><html><body><div id=root></div></body></html>", {
  url: "http://localhost:5202/?variant=A",
...
const { Daylight } = await import("./src/variants/Daylight");
const { Nightfall } = await import("./src/variants/Nightfall");
const { Graphite } = await import("./src/variants/Graphite");
const { snapshot, advanceFrontier } = await import("./src/lib/fixture");

const variants = { A: Daylight, B: Nightfall, C: Graphite } as const;
...
// runs. jsdom has no SVG geometry or WAAPI, so this proves the React logic.
{
  ...
  root.render(createElement(Nightfall, { snap: snapshot }));
  await new Promise((r) => setTimeout(r, 150));
  const before = count();

  root.render(createElement(Nightfall, { snap: advanceFrontier(snapshot) }));

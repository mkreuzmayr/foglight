...
const dom = new JSDOM("<!doctype html><html><body><div id=root></div></body></html>", {
  url: "http://localhost:5203/?variant=B",
...
const { Nightfall } = await import("./src/variants/Nightfall");
const { VioletHour } = await import("./src/variants/VioletHour");
const { Harbor } = await import("./src/variants/Harbor");
const { Ember } = await import("./src/variants/Ember");
const { snapshot, advanceFrontier } = await import("./src/lib/fixture");

const variants = { A: Nightfall, B: VioletHour, C: Harbor, D: Ember } as const;
...
// runs. jsdom has no SVG geometry or WAAPI, so this proves the React logic.
{
  ...
  root.render(createElement(VioletHour, { snap: snapshot }));
  await new Promise((r) => setTimeout(r, 150));
  const before = count();

  root.render(createElement(VioletHour, { snap: advanceFrontier(snapshot) }));
  ...
  console.log(`\ntransition B: before ${before}n → during ${during}n (ghosts) → after ${after}n`);

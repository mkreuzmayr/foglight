...
const dom = new JSDOM("<!doctype html><html><body><div id=root></div></body></html>", {
  url: "http://localhost:5204/?variant=B",
...
const { Rose } = await import("./src/variants/Rose");
const { Azure } = await import("./src/variants/Azure");
const { Graphite } = await import("./src/variants/Graphite");
const { snapshot, advanceFrontier } = await import("./src/lib/fixture");

const variants = { A: Rose, B: Azure, C: Graphite } as const;
...
// runs. jsdom has no SVG geometry or WAAPI, so this proves the React logic.
{
  ...
  root.render(createElement(Rose, { snap: snapshot }));
  await new Promise((r) => setTimeout(r, 150));
  const before = count();

  root.render(createElement(Rose, { snap: advanceFrontier(snapshot) }));

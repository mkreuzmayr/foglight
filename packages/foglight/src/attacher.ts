...
import { decodeLine, encode, ShutdownRequest } from "./protocol.js";
...
const lineReader = (socket: Socket) => {
  type ReaderState = { buf: string; closed: Error | null };
  const readerState: ReaderState = { buf: "", closed: null };
  const waiters: ((line: Effect.Effect<string, Error>) => void)[] = [];

  const flush = () => {
    while (waiters.length > 0) {
      if (readerState.closed !== null) {
        waiters.shift()?.(Effect.fail(readerState.closed));
        continue;
      }

      const nl = readerState.buf.indexOf("\n");
      if (nl < 0) {
        break;
      }

      const line = readerState.buf.slice(0, nl + 1);
      readerState.buf = readerState.buf.slice(nl + 1);
  ...
  socket.on("close", () => {
    readerState.closed = new Error("socket closed");
    flush();
  });

  socket.on("error", (error) => {
    readerState.closed = error;
    flush();
  });
...
const printEvent = (input: AttacherInput, message: Message): void => {
  if (message.type === "event") {
    if (message.kind === "daemon-exiting") {
      input.write("  daemon exiting");
...
const session = (input: AttacherInput, socket: Socket): Effect.Effect<number, unknown> =>
  Effect.gen(function* () {
    ...
    if (reply.type !== "hello-reply" || !reply.ok) {
      input.writeError(handshakeError(reply));
    ...
    if (warning !== null) {
      input.writeError(warning);
    }

    input.write(`  ${reply.url}`);
    if (input.printQr !== undefined) {
      yield* input.printQr(reply.url);
    }
...
export const runAttacher = (
  ...
  Effect.gen(function* () {
    ...
    type Startup = { spawnedAt: number | null };
    const startup: Startup = { spawnedAt: null };
    for (;;) {
      ...
      if (opened._tag === "Right") {
        ...
        if (input.mode === "status" || code !== 0) {
          return code;
        }

        startup.spawnedAt = null;
        continue;
      }
      ...
      if (startup.spawnedAt !== null && Date.now() - startup.spawnedAt > 8_000) {
      ...
      if (yield* acquireClaim(input.layout.claimPath)) {
        ...
        startup.spawnedAt = Date.now();
      } else {
        yield* clearDeadClaim(input.layout.claimPath);
      }
...
const isVersionSkew = (reply: Message) =>
  reply.type === "hello-reply" && !reply.ok && reply.error === "version-skew";

const handshakeError = (reply: Message) =>
  reply.type === "hello-reply" && !reply.ok
    ? `foglight: ${reply.message}`
    : "foglight: handshake failed";

const clearDeadClaim = (path: string) =>
  Effect.gen(function* () {
    const record = yield* readClaim(path);
    if (record !== null && !isProcessAlive(record.pid)) {
      yield* releaseClaim(path);
    }
  });
